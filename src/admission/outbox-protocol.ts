...
const CAPABILITY_FIELDS = ["versions", "required", "ackRequests", "durableEventIdDedupe", "reconnectReplay"] as const;
...
/**
 * Accept only an exact peer offer that proves v1 durable dedupe, ACK handling,
 * and reconnect replay. Any unrecognized shape fails closed to no negotiation.
 */
export function negotiateOutboxCapability(offer: unknown): OutboxCapability | null {
  try {
    ...
    if (fields.reconnectReplay !== true) return null;
