...
/** Connector session inventory projected into the controller recovery contract. */
export interface StartupRecoveryActiveSessionRecord {
  readonly agentId: string;
  readonly sessionId: string;
  readonly requestId: string;
  readonly conversationId: string | null;
  readonly cursor: number;
  readonly connectorIdentity: VerifiedLinuxConnectorIdentity;
  readonly leaseGeneration: number;
  readonly terminalState: "completed" | "failed" | "cancelled" | null;
}
...
export interface StartupRecoveryBarrierSources {
  ...
  listActiveSessions():
    | readonly StartupRecoveryActiveSessionRecord[]
    | PromiseLike<readonly StartupRecoveryActiveSessionRecord[]>;
  listRecoverableOutboxClaims():
...
function sourceInventoryIssues(
  ...
  sessions: InventoryRead<StartupRecoveryActiveSessionRecord>,
...
function reconcileDurableInventories(
  ...
  sessions: readonly StartupRecoveryActiveSessionRecord[],
...
function normalizeActiveSessions(value: unknown): readonly StartupRecoveryActiveSessionRecord[] | null {
  if (!Array.isArray(value)) return null;
  const normalized: StartupRecoveryActiveSessionRecord[] = [];
  for (const item of value) {
...
function isRecoverableClaimState(value: unknown): value is DeliveryClaimLease["state"] {
  return value === "claimed" || value === "recovery_required";
}
