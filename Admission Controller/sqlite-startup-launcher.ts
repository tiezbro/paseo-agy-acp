...
/** Structural process classes understood by the controller startup gate. */
export type AdmissionStartupClass = "model_turn" | "auxiliary" | "resident_pty";

/** Structural permit contract consumed by the Connector's agy launcher. */
export interface AdmissionStartupPermit {
  release(): void;
}

/** Controller-owned startup gate, independent of any process implementation. */
export interface AdmissionStartupLauncher {
  readonly enabled: boolean;
  acquire(classification: AdmissionStartupClass): AdmissionStartupPermit;
}
...
export interface HeldAgyStartupPermit extends AdmissionStartupPermit {
 ...
 */
export class SqliteAgyStartupLauncher implements AdmissionStartupLauncher {
  ...
  acquire(classificationInput: AdmissionStartupClass): AdmissionStartupPermit | HeldAgyStartupPermit {
    this.assertOpen();
...
const NOOP_MODEL_START_PERMIT: AdmissionStartupPermit = Object.freeze({ release() {} });
...
function requireStartupClass(value: unknown): AdmissionStartupClass {
