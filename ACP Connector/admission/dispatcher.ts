...
export interface AdmissionPromptDispatcherOptions<TProcess, TProcessIdentity> {
  ...
  readonly now?: () => number;
  /** Zero keeps one-shot unit seams; production uses a bounded polling interval. */
...
export class AdmissionPromptDispatcher<TProcess, TProcessIdentity> {
  ...
  constructor(options: AdmissionPromptDispatcherOptions<TProcess, TProcessIdentity>) {
    ...
    this.#admissionPollIntervalMs = normalizePollInterval(options.admissionPollIntervalMs);
  ...
  private async runOnce(input: AdmissionPromptDispatchInput, requestId: string): Promise<AdmissionPromptDispatchOutcome> {
    ...
    if (lease === null && this.#admissionPollIntervalMs > 0) {
      const waited = await this.waitForAdmission(input, requestId);
      if ("state" in waited) return waited;
      lease = waited.lease;
      now = waited.now;
    }
  ...
  private async waitForAdmission(
    ...
    | Readonly<{ lease: AdmissionLease; now: number }>
    | Extract<AdmissionPromptDispatchOutcome, { readonly state: "failed" | "cancelled" | "recovery_required" }>
  > {
    for (;;) {
      if (this.#closed) {
        return Object.freeze({ state: "recovery_required", reason: "unexpected_fault" });
      ...
      if (isClaimCancelled(input)) {
        this.cancelQueued(requestId);
        return Object.freeze({ state: "cancelled", stopReason: "cancelled" });
      }

      const now = this.now();
      const state = this.#controller.getRequest?.(requestId)?.state;
      if (state === "queue_timeout") return Object.freeze({ state: "failed", stopReason: "end_turn" });
      if (state === "cancelled") return Object.freeze({ state: "cancelled", stopReason: "cancelled" });
      if (state !== undefined && state !== "queued") {
        return Object.freeze({ state: "recovery_required", reason: "unexpected_fault" });
      }

      try {
        const snapshot = this.#controller.getQueueSnapshot?.(requestId, now);
        if (snapshot !== null && snapshot !== undefined) input.reportQueueProgress?.(snapshot);
      } catch {
        // Queue observation cannot mutate or roll back the durable request.
      }

      if (!(await waitForAdmissionPoll(this.#admissionPollIntervalMs, input.claim.signal))) {
        this.cancelQueued(requestId);
        return Object.freeze({ state: "cancelled", stopReason: "cancelled" });
      }
      ...
      try {
        const lease = this.#controller.admitRequest(requestId, attemptAt, this.#ownerInstanceId);
        if (lease !== null) return Object.freeze({ lease, now: attemptAt });
      } catch {
        return Object.freeze({ state: "recovery_required", reason: "admission_failed" });
      }
    }
  }

  private cancelQueued(requestId: string): void {
    try {
      this.#controller.cancelQueued?.(requestId, this.now());
    } catch {
      // A concurrent lease transition must not be rewritten as queued cancellation.
    }
  }
  ...
  private async startAndObserveIssued(
    ...
    try {
      this.#controller.markProviderTerminal(lease, now, toConfirmedProviderTerminal(confirmation), {
        ...terminal.delivery,
...
function normalizePollInterval(value: unknown): number {
  ...
  if (!Number.isSafeInteger(value) || (value as number) < 0 || (value as number) > 60_000) {
    throw new Error("admission dispatcher admissionPollIntervalMs must be an integer from 0 through 60000");
  }
  return value as number;
}

function waitForAdmissionPoll(milliseconds: number, signal: AbortSignal): Promise<boolean> {
