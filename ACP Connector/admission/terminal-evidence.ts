...
/** Protocol-neutral terminal fact accepted by the Admission Controller. */
export interface ConfirmedProviderTerminalRecord {
  readonly outcome: "completed" | "failed" | "cancelled";
  readonly conversationId: string;
  readonly status: OfficialTerminalStatus;
  readonly streamObservedAt: number | null;
  readonly sqliteObservedAt: number;
  readonly failure: Readonly<ClassifiedProviderFailure> | null;
}

export function toConfirmedProviderTerminal(
  confirmation: Exclude<TerminalEvidenceConfirmation, TerminalEvidenceRecoveryRequired>
): ConfirmedProviderTerminalRecord {
  return Object.freeze({
    outcome: confirmation.status === "SUCCESS"
      ? "completed"
    ...
    streamObservedAt: confirmation.streamJson?.observedAt ?? null,
    sqliteObservedAt: confirmation.sqliteReconciliation.observedAt,
    failure: confirmation.sqliteReconciliation.outcome === "failed"
      ? confirmation.sqliteReconciliation.failure
      : null
  });
}
