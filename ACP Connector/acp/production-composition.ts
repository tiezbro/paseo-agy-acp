...
export async function composeAcpProductionRuntime<TProcessIdentity>(
  ...
  try {
    ...
    resources.admission = dependencies.runtime.createPromptSeam({
      ...
      reportQueueProgress: (progress) => resources.clientRoutes!.resolve({
        sessionId: progress.sessionId,
        protocol: dependencies.clientRoute.protocol,
        connectionFence: dependencies.clientRoute.connectionFence
      }).send(queueProgressUpdate(progress)),
      now: dependencies.now
...
function queueProgressUpdate(progress: {
  ...
  return Object.freeze({
    ...
    ...(progress.position === undefined ? {} : { position: progress.position }),
    ...(progress.eligiblePosition === undefined ? {} : { eligiblePosition: progress.eligiblePosition }),
    ...(progress.waitedMs === undefined ? {} : { waitedMs: progress.waitedMs }),
    ...(progress.cooldownUntil === undefined ? {} : { cooldownUntil: progress.cooldownUntil })
  });
}
