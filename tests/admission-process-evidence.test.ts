...
describe("Linux admission process evidence", () => {
  ...
  it("rejects unavailable, malformed, mismatched, or out-of-range process evidence", () => {
    ...
    expect(() => captureLinuxProcessIdentity(PID, readers(procStat({ ppid: 0 })))).toThrow(ProcessEvidenceError);
    expect(() => captureLinuxProcessIdentity(PID, readers(procStat({ pgrp: 0 })))).toThrow(ProcessEvidenceError);
    expect(() => captureLinuxProcessIdentity(PID, readers(procStat({ session: 0 })))).toThrow(ProcessEvidenceError);
